...
export default function Notes() {
  ...
  const handleDelete = (noteId) => {
    ...
    //   axios.delete(`${baseUrl}/${noteId}`).then((response) => {
    //     console.log(response);
    //     setNotes(notes.filter((n) => n.id !== noteId));
    //   });
    // }
    axios.delete(`${
      baseUrl}/${noteId}`).then((response) => {
      console.log(response);
      setNotes(notes.filter((n) => n.id !== noteId));
    });
  };
    ...
    axios
      ...
      .then((response) => {
        ...
        const updatedNotes = notes.map((n) =>
          n.id === targetNote.id ? { ...targetNote, desc: desc } : n
        );
      });
  ...
  return (
    <>
      <h1>Notes App</h1>
      <h2 style = {{color : "red" , fontstyle :"italica" , fontSize: 10 }}> Tadaaaaaa el stupido </h2>
